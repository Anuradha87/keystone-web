...
const Popup = ({ repo, onClose, handleRemoveClick }) => {
    ...
    <div className="popup">
      <div className="popup-inner">
            ...
            {Array.isArray(repo) && repo.length > 0 ? (
              repo.map((item, index) => (
                <li key={item.id}>
                  ...
                  <span className="close-icon" onClick={() => handleRemoveClick(item)}>X</span>
                </li>
              ))
            ) : (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={repo.length === 0}>Submit</button>
